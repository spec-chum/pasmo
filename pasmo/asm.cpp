...
const byte codeADDBC= 0x33;
const byte codeADDDE= 0x32;
...
private:
    ...
    void parseSUBDEHL (Tokenizer & tz);
    void parseADDBCDE (Tokenizer & tz, byte prefix, byte basecode);
    void parseADDDEHL (Tokenizer & tz);
...
void Asm::In::parseADDBCDE (Tokenizer & tz, byte prefix, byte basecode)
{
	...
	case TypeA:
        no86 ();
        gencodeED (basecode);
        showcode ("ADD " + std::string(basecode == codeADDBC ? "BC" : "DE") + ", A");
        no8080 ();
		break;
    case TypeNumber:
        {
...
void Asm::In::parseADD (Tokenizer & tz)
{
	...
	case TypeA:
		...
		break;
	case TypeBC:
		parseADDBCDE (tz, prefixED, codeADDBC);
		return;
	case TypeDE:
		parseADDBCDE (tz, prefixED, codeADDDE);
		return;
	case TypeHL:
